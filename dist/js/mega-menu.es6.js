...
var MegaMenu = /*#__PURE__*/function () {
   ...
   * @return {void}
   */
  function MegaMenu(domNode, _temp) {
    ...
    if (this.useArrowKeys) {
      this.topLevelItems = [].concat(this.topLevelItems, Array.from(this.menu.querySelectorAll('a.js-top-level')));
    }
  ...
  _proto.toggleSection = function toggleSection(section, hide) {
    if (hide) {
      section.hidden = true;
    } else {
    ...
    var focusable = section.querySelectorAll('button, [href], input, select, textarea, [tabindex]');
    focusable.forEach(function (focusableItem) {
  ...
  _proto.toggleExpand = function toggleExpand(index, expanded) {
  ...
  _proto.controlFocusByKey = function controlFocusByKey(event, menuLinks, currentIndex) {
    switch (event.key) {
      case 'ArrowUp':
      case 'ArrowLeft':
  ...
  _proto.handleSectionKeydown = function handleSectionKeydown(event) {
    if (this.openIndex === null || !this.useArrowKeys) return;
    var menuLinks = [].concat(this.menuSections[this.openIndex].querySelectorAll('a, button'));
  ...
  _proto.handleButtonClick = function handleButtonClick(event) {
    ...
    var buttonIndex = this.topLevelItems.indexOf(currentTarget);
  ...
  _proto.handleButtonKeydown = function handleButtonKeydown(event) {
    if (!this.useArrowKeys) return;
    var targetButtonIndex = this.topLevelItems.indexOf(document.activeElement);
  ...
  _proto.handleLinkKeydown = function handleLinkKeydown(event) {
    if (!this.useArrowKeys) return;
    var targetLinkIndex = this.topLevelItems.indexOf(document.activeElement);
  ...
  _proto.handleCloseClick = function handleCloseClick(event) {
    event.preventDefault();
    this.toggleExpand(this.openIndex, false);
  }
  /**
  ...
  _proto.prepSection = function prepSection(button) {
    var section = button.parentNode.querySelector('.c-mega-menu__section');
    if (!section) return;
    var closeButton = section.querySelector('.c-mega-menu__section-close');
  ...
  _proto.prepLink = function prepLink(link) {
  ...
  _proto.handleClickAnywhere = function handleClickAnywhere(event) {
  ...
  _proto.handleKeydownAnywhere = function handleKeydownAnywhere(event) {
    var _this = this;
    ...
    if (event.key === 'Escape' && this.openIndex !== null) {
      ...
      setTimeout(function () {
        if (document.activeElement.classList.contains('js-top-level')) {
          var openButton = _this.menu.querySelector('button[aria-expanded="true"]');

          if (openButton) {
            var buttonIndex = _this.topLevelItems.indexOf(openButton);

            _this.toggleExpand(buttonIndex, false);
          }
        }
      }, 0);
    }
  ...
  _proto.init = function init() {
    ...
    this.topLevelItems.forEach(function (item) {
      if (item.tagName === 'BUTTON') {
        _this2.prepSection(item);
      } else {
        _this2.prepLink(item);
      }
    });
  };
...
/* harmony import */ var _modules_MegaMenu_es6__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modules/MegaMenu.es6 */ "./source/03-components/mega-menu/modules/MegaMenu.es6.js");
/* harmony import */ var _mobile_menu_modules_MobileMenu_es6__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../mobile-menu/modules/_MobileMenu.es6 */ "./source/03-components/mobile-menu/modules/_MobileMenu.es6.js");





(drupal__WEBPACK_IMPORTED_MODULE_2___default().behaviors.megaMenu) = {
  attach: function attach(context) {
    var menus = context.querySelectorAll('.c-mega-menu');

    if (menus.length) {
      menus.forEach(function (menu) {
        var megaMenu = new _modules_MegaMenu_es6__WEBPACK_IMPORTED_MODULE_3__["default"](menu);
        megaMenu.init();
        var mobileMenu = new _mobile_menu_modules_MobileMenu_es6__WEBPACK_IMPORTED_MODULE_4__["default"](menu, context, {

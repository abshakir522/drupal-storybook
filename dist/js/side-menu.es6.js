...
/* harmony import */ var _overlay_menu_modules_OverlayMenu_es6__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../../overlay-menu/modules/_OverlayMenu.es6 */ "./source/03-components/overlay-menu/modules/_OverlayMenu.es6.js");
...
var SideMenu = /*#__PURE__*/function (_OverlayMenu) {
  _inheritsLoose(SideMenu, _OverlayMenu);
   ...
   */
  function SideMenu(domNode, _temp) {
    var _this;
    ...
    _this = _OverlayMenu.call(this, domNode, {
  ...
  var _proto = SideMenu.prototype;

  _proto.createBackButton = function createBackButton(submenu) {
    var _this2 = this;

    var submenuBack = document.createElement('button');
    submenuBack.classList.add('c-side-menu__back');
    submenuBack.innerText = 'Back';
    submenuBack.addEventListener('click', function () {
      _this2.closeSubmenu(submenu);
    });
    return submenuBack;
  }
  /**
  ...
  _proto.createSubmenuTitle = function createSubmenuTitle(title) {
  ...
  _proto.createSubmenuToggle = function createSubmenuToggle(submenu) {
    var _this3 = this;
  ...
  _proto.closeSubmenu = function closeSubmenu(submenu) {
    submenu.classList.remove('is-open');
    var currentSection = submenu.closest('.is-open');
  ...
  _proto.openSubmenu = function openSubmenu(submenu) {
    var currentSection = submenu.closest('.is-open');

    if (currentSection) {
      var focusableItems = currentSection.querySelectorAll('button:not(.c-hamburger-button--close), [href], input, select, textarea');
  ...
  _proto.prepSubmenu = function prepSubmenu(topLevelItem) {
    var submenu = topLevelItem.parentElement.querySelector('.c-menu__subnav');
    if (!submenu) return null;
    var submenuBack = this.createBackButton(submenu);
    submenu.insertAdjacentElement('afterbegin', submenuBack);
    var submenuTitle = this.createSubmenuTitle(topLevelItem.innerText);
  ...
  _proto.prepButton = function prepButton(button) {
    var _this4 = this;

    this.menuItems.push(button);
    var submenu = this.prepSubmenu(button);
    button.addEventListener('click', function () {
      _this4.openSubmenu(submenu);
    });
  }
  /**
  ...
  _proto.prepLink = function prepLink(link) {
    ...
    if (link.classList.contains('has-subnav')) {
      var submenu = this.prepSubmenu(link);
      var submenuToggle = this.createSubmenuToggle(submenu);
  ...
  _proto.init = function init() {
    ...
    var menuItems = document.querySelectorAll('.c-menu__link');
    menuItems.forEach(function (item) {
      if (item.tagName === 'BUTTON') {
        _this5.prepButton(item);
      } else if (item.tagName === 'A') {
        _this5.prepLink(item);
      } else {
  ...
  _proto.enableTab = function enableTab(startingPoint) {
    ...
    var subNavs = startingPoint.querySelectorAll('.c-menu__subnav');
    subNavs.forEach(function (subNav) {
      if (!subNav.classList.contains('is-open')) {
        var subnavItems = subNav.querySelectorAll('button, [href], input, select, textarea, [tabindex]');
  ...
  _proto.handleKeydown = function handleKeydown(event) {
    ...
    if (key === 'Tab' || key === 'Escape') {
      _OverlayMenu.prototype.handleKeydown.call(this, event);
    } else {
      if (!this.options.useArrowKeys) return;
      var focusable = [].concat(this.overlay.querySelectorAll('button, [href], input, select, textarea')).filter(function (item) {
        return item.tabIndex !== -1;
      });
      var numberFocusElements = focusable.length;
      var firstFocusableElement = focusable[0];
      ...
      switch (key) {
        ...
        case 'ArrowDown':
          if (document.activeElement === lastFocusableElement) {
            ...
            var _currentIndex = focusable.indexOf(document.activeElement);

            if (_currentIndex !== -1) {
              focusable[_currentIndex + 1].focus();
            }
          }
        ...
        case 'ArrowRight':
          if (document.activeElement.closest('.c-menu__item')) {
            var activeMenuItem = document.activeElement.closest('.c-menu__item');

            if (activeMenuItem.classList.contains('has-subnav')) {
              var submenu = activeMenuItem.querySelector('.c-menu__subnav');
        ...
        case 'ArrowLeft':
          if (document.activeElement.closest('.c-menu__item')) {
            var _activeMenuItem = document.activeElement.closest('.c-menu__item');

            if (_activeMenuItem.classList.contains('has-subnav')) {
              var _submenu = _activeMenuItem.querySelector('.c-menu__subnav');

              this.closeSubmenu(_submenu);
            }
          }
  ...
  return SideMenu;
}(_overlay_menu_modules_OverlayMenu_es6__WEBPACK_IMPORTED_MODULE_4__["default"]);
...
/* harmony import */ var _modules_SideMenu_es6__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modules/_SideMenu.es6 */ "./source/03-components/side-menu/modules/_SideMenu.es6.js");




(drupal__WEBPACK_IMPORTED_MODULE_2___default().behaviors.sideMenu) = {
  attach: function attach(context) {
    var menus = context.querySelectorAll('.c-side-menu');

    if (menus.length) {
      menus.forEach(function (menu) {
        var sideMenu = new _modules_SideMenu_es6__WEBPACK_IMPORTED_MODULE_3__["default"](menu);
        sideMenu.init();
      });
    }
  }
